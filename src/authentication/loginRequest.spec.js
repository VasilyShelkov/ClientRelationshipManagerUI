...
describe('src/authentication/loginRequest.js', () => {
  it('logs in successfully', sinon.test(async function () {
    const values = {
  ...
  it('attempts to login but service fails', sinon.test(async function () {
    const values = {
      email: 'testEmail@test.com',
      password: '1234'
    };
    ...
    await expect(loginRequest(values, dispatch, props))
