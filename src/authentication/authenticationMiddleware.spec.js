...
describe('src/authentication/authenticationMiddleware.js', () => {
  it('calls next middleware', sinon.test(function () {
  ...
  it('does not do anything if the route does not require login', sinon.test(function () {
  ...
  it('redirects to the login page if the route requires login and the user is not logged in', sinon.test(function () {
  ...
  it('does not do anything if the route requires login but the user is logged in', sinon.test(function () {
    const { dispatch, store, next, action } = setup({
      pathname: '/account/profile',
      storeState: {
