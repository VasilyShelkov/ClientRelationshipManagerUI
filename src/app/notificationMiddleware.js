...
import { APOLLO_MUTATION_RESULT, showSuccessNotification } from './thirdPartyActions';
    ...
    if (!_.has(action, 'result.errors')) {
      switch (action.operationName) {
        case 'CreateUnprotectedName': {
          ...
          store.dispatch(showSuccessNotification({ title: 'Created unprotected', firstName, lastName }));
          break;
        }
        case 'ProtectName': {
          const { firstName, lastName } = action.result.data.protectNameToUser.name;
          store.dispatch(showSuccessNotification({ title: 'Protected name', firstName, lastName }));
          break;
        }
        case 'MetWithProtected': {
          const { firstName, lastName } = action.result.data.editProtectedName.name;
          store.dispatch(showSuccessNotification({ title: 'Met with protected', firstName, lastName }));
          break;
        }
        case 'MakeClient': {
          const { firstName, lastName } = action.result.data.addClientToUser.name;
          store.dispatch(showSuccessNotification({ title: 'Congrats on the new client', firstName, lastName }));
          break;
        }
        case 'UnprotectName': {
          const { firstName, lastName } = action.result.data.unprotectNameFromUser.name;
          store.dispatch(showSuccessNotification({ title: 'Unprotected name', firstName, lastName }));
          break;
        }
      }
    }
